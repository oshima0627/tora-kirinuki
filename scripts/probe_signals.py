...
ヒートマップだけは YouTube Data API でも yt-dlp でも取れず、動画ページの
ytInitialData にしか無いので、ブラウザで取った JSON を --ytdata で渡す。
無くても動く（令和の虎Second では30本中7本にしか存在しない）。
"""
...
def main() -> None:
    ...
    ap.add_argument("--ytdata", type=Path,
                    help="ブラウザで取った ytInitialData の JSON（ヒートマップ用）")
