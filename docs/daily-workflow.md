...
**最有力の仮説：長尺の冒頭4秒が無音の静止カードであること。**
`build_clip.py` は `[案件カード, 本編, 判定カード]` の順に連結し、カードは
`_still()` が `anullsrc` で無音トラックを付けて作る。実測すると冒頭4秒は
-91dB（完全な無音）で、そのあいだ画面は動かない。新規チャンネルの初回配信は
冒頭数秒の維持率で決まるので、ここが効いている可能性が高い。

直すなら案件カードを本編の頭に**オーバーレイ**する（論点カードと同じ方式）か、
本編を数秒流してからカードを挟む形にする。どちらも `build_clip.py` の
連結順の変更で、既存8本は作り直しと再アップロードになる（動画IDが変わる）。

**まだ検証していない。** 3日で8本という母数では、冒頭の問題なのか
単に新規チャンネルの初期状態なのかを分離できない。片方だけ直した動画を出して
比べるのが確実。
