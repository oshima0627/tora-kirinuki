...
def test_ASRの崩れをレシピの訂正表で直す():
    # 実測で「阿久津さん→その悪さん」「焼き鳥3級→焼き鳥産」と崩れる。
    # 数字が無いので risky_lines では拾えず、そのまま焼かれてしまう
    c = cues((100.0, "その悪さんが今日全然元気なく"), (105.0, "自分はもう焼き鳥産で"))
    plan = burn_plan(c, 100.0, 110.0, fixes={"その悪": "阿久津", "焼き鳥産": "焼き鳥3級"})
    assert [p["text"] for p in plan] == [
        "阿久津さんが今日全然元気なく", "自分はもう焼き鳥3級で"]


def test_訂正表が無ければ何も変わらない():
    c = cues((100.0, "その悪さんが今日全然元気なく"))
    assert burn_plan(c, 100.0, 110.0)[0]["text"] == "その悪さんが今日全然元気なく"


def test_使われなかった訂正を名指しする():
...
def test_キューをまたぐ訂正は当たらないので名指しする():
    # **訂正はキュー単位で当てている。** ASRは「年少の高い」「社長なんですよ。」の
    # ように語の途中で切るので、またぐ訂正は一度も当たらない。焼いた字幕の
    # 連結だけを見ると繋がって見えてしまい、当たったように誤判定していた
    # （yotsui-kakkoii で実測）
    from scripts.subtitles import unused_fixes
    c = cues((100.0, "四ツ井さんは年少の高い"), (102.0, "社長なんですよ。"))
    assert unused_fixes(c, 100.0, 110.0, {"年少の高い社長": "年商の高い社長"})         == ["年少の高い社長"]
    assert unused_fixes(c, 100.0, 110.0, {"年少の高い": "年商の高い"}) == []
